...
def get_providers(connect_url, db, is_for_subscription=False, enabled_tiers=None, preferred_protocol=None):
    servers = get_all_servers(db=db)
    
    # # DEBUG ONLY
    # servers.append((config.SERVER_MAIN_IP, 443))

    health_data = health_check.get_health_cache()
    ...
    for server, port, tier, server_entry_type in servers:
        ...
        for s in server_ips:
            ...
            score = 0
            server_health = [x for x in health_data if x['_id'] == server]
            if len(server_health) > 0:
                score = server_health[0]['averageSuccessRate']
            ...
            if settings.get_provider_enabled('trojanws', db=db):
                provider_infos['trW'] = init_provider_info(
                    type='trojan-ws',
            ...
            if settings.get_provider_enabled('vlessws', db=db):
                provider_infos['vlW'] = init_provider_info(
                    type='vless-ws',
            ...
            if settings.get_provider_enabled('trojangrpc', db=db):
                provider_infos['trG'] = init_provider_info(
                    type='trojan-grpc',
            ...
            if settings.get_provider_enabled('vlessgrpc', db=db):
                if server_entry_type != 'SecondaryProxy' or is_for_subscription:
                    ...
                    provider_infos['vlG'] = init_provider_info(
                        type='vless-grpc',
            ...
            if settings.get_provider_enabled('vmessgrpc', db=db):
                provider_infos['vmG'] = init_provider_info(
                    type='vmess-grpc',
            ...
            if settings.get_provider_enabled('ssv2ray', db=db):
                if not is_for_subscription or server_entry_type != 'SecondaryProxy':
                    ...
                    provider_infos['ssW'] = init_provider_info(
                        type='ss-v2ray',
            ...
            if preferred_protocol is not None:
                if preferred_protocol in provider_infos and provider_infos[preferred_protocol] is not None:
                    providers.append(provider_infos[preferred_protocol])
            for key in protocols:
                if key in provider_infos and provider_infos[key] is not None and key != preferred_protocol:
                    providers.append(provider_infos[key])
            
            idx += 1

    server_groups.sort(key=lambda x: x['score'], reverse=True)
...
def generate_conf_singlefile(user_id, connect_url, meta=False, premium=False, enabled_tiers=None, tier_type_override=None, add_country_rules=True, preferred_protocol=None, custom_info_entries=None):
    ...
    providers = get_providers(connect_url, db, enabled_tiers=enabled_tiers, preferred_protocol=preferred_protocol)
    ...
    for i in range(4):
        tiers.append({
            ...
            'type': settings.get_tier_proxygroup_type(i+1, db=db) if tier_type_override is None else tier_type_override,
        })
    ...
    clash_custom_rules = _parse_clash_custom_rules(outbounds_module.get_clash_custom_rules())
